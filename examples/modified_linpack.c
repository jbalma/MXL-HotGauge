...
** - ANSI prototyping.
**
** To compile:  gcc -O -o mod_linpack modified_linpack.c -lm
**
**
*/
...
int main(void)
{
    int     arsize = 1000;   /* choose your fixed problem size here */
    long    arsize2d, memreq, nreps;
    size_t  malloc_arg;

    /* make sure array size is even, preserving original behavior */
    arsize /= 2;
    arsize *= 2;
    ...
    memreq = arsize2d * sizeof(REAL)
           + (long)arsize * sizeof(REAL)
           + (long)arsize * sizeof(int);

    printf("Memory required:  %ldK.\n", (memreq + 512L) >> 10);

    malloc_arg = (size_t)memreq;
    if (malloc_arg != (size_t)memreq || (mempool = malloc(malloc_arg)) == NULL)
    {
    ...
    nreps = 1;
    while (linpack(nreps, arsize) < 10.0)
        nreps *= 2;
