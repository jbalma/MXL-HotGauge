...
)
{
    switch (ipoint->OType)
    {
        ...
        case TDICE_OUTPUT_TYPE_TFLP :
        {
            ...
            {
                /* MXL fix: insert_message_word() memcpy's exactly sizeof(MessageWord_t)
                 * = 4 bytes, but Temperature_t is a double -- so this sent the LOW HALF of
                 * each double's mantissa and every Tflp temperature over the socket was
                 * garbage (NaN read as float, ~1e307 read as double). Every other branch in
                 * this file already declares a float local first (lines ~831, ~969, ~1002);
                 * this one did not. Cast explicitly, matching them. */
                float t = (float) tmp [ index ] ;
