...
#define SERVER_PORT  argv[2]
    ...
    /* MXL: steady analysis is allowed too.
     *
     * The whole point of the server is that thermal_data_build() factorises the system matrix
     * ONCE and every later request reuses it. Measured on a 34-core die (691k unknowns):
     * factorisation ~83 s, each solve <0.3 s. The stock Emulator re-runs as a fresh process
     * per iteration, so a 51-solve leakage/MR study refactors an unchanged matrix 51 times --
     * 70 minutes of work that a persistent server does in under two.
     *
     * emulate_steady() already exists and reuses the factorisation; only this guard and the
     * result handling below stood in the way. */
    if (analysis.AnalysisType != TDICE_ANALYSIS_TYPE_TRANSIENT
     && analysis.AnalysisType != TDICE_ANALYSIS_TYPE_STEADY)
    {
        fprintf (stderr, "only transient or steady analysis!\n") ;
    ...
    do
    {
        ...
        /**********************************************************************/
            ...
            case TDICE_SIMULATE_SLOT :
            {
                ...
                /* MXL: dispatch on analysis type. emulate_steady() returns
                 * TDICE_END_OF_SIMULATION on SUCCESS -- for a steady solve that means "done",
                 * not "shut down" -- so it is normalised to TDICE_SLOT_DONE before the reply,
                 * otherwise the server would exit after answering a single request and we
                 * would be back to one factorisation per solve. */
                SimResult_t result ;

                if (analysis.AnalysisType == TDICE_ANALYSIS_TYPE_STEADY)
                {
                    result = emulate_steady (&tdata, stkd.Dimensions, &analysis) ;

                    if (result == TDICE_END_OF_SIMULATION)

                        result = TDICE_SLOT_DONE ;
                }
                else

                    result = emulate_slot (&tdata, stkd.Dimensions, &analysis) ;
