     ...
     sinkTemperatures(nullptr,0,0), sinkHeatFlows(nullptr,0,0)
{
    ...
    // --- DIAGNOSTIC: what did we actually read out of the FMU? ---
    cout<<"\n[diag] sinkConductance="<<sinkConductance
        <<" sinkLength(um)="<<sinkLength<<" sinkWidth(um)="<<sinkWidth
        <<" sinkRows="<<sinkRows<<" sinkColumns="<<sinkColumns
        <<"\n[diag] spreaderX0(um)="<<spreaderX0<<" spreaderY0(um)="<<spreaderY0
        <<" spreaderConductance="<<spreaderConductance
        <<" cellLength="<<cellLength<<" cellWidth="<<cellWidth
