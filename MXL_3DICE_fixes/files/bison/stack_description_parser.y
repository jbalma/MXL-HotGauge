...
solver
  ...
  : SOLVER ':'
...

    {
        ...
        // Init the pluggable heatsink now: StepTime and the initial temperature are both set.
        if(stkd->TopHeatSink && stkd->TopHeatSink->SinkModel == TDICE_HEATSINK_TOP_PLUGGABLE)
        {
  ...
  | SOLVER ':'
        ...
        INITIAL_CONDITIONS
    {
        ...
        // Init the pluggable heatsink now, NOT in INITIAL_CONDITIONS: StepTime is only
        // assigned above, and the plugin needs it as its co-simulation timestep.
        if(stkd->TopHeatSink && stkd->TopHeatSink->SinkModel == TDICE_HEATSINK_TOP_PLUGGABLE)
        {
            if(initialize_pluggable_heatsink(stkd->TopHeatSink, analysis)!=TDICE_SUCCESS)
                YYABORT ;
  ...
  : INITIAL_ SINK TEMPERATURE DVALUE ';' // $4
    {
        ...
        // NOTE (2026-08-12): the pluggable-heatsink init used to happen HERE. It cannot:
        // INITIAL_CONDITIONS reduces BEFORE the enclosing solver action that assigns
        // analysis->StepTime, so the plugin was always initialised with StepTime = 0. The
        // FMU was then stepped with dt = 0 forever -- it never advanced, the sink stayed at
        // its initial temperature, and the heatsink appeared to reject no heat at any fan
        // speed. Moved to the end of the steady/transient solver actions, where StepTime is
        // set and InitialHeatsinkTemperature (assigned here) is already available.
