...
def single_mcpat_to_blk_lvl_power_dict(mcpat_json_file, num_cores, write=True, emit_split=True):
    ...
    # leakages?
    per_core_units = [
        'Execution Unit/Complex ALUs',
        'Execution Unit/Floating Point Units',
        'Execution Unit/Instruction Scheduler/FP Instruction Window',
        'Execution Unit/Instruction Scheduler/Instruction Window',
        'Execution Unit/Instruction Scheduler/ROB',
        'Execution Unit/Instruction Scheduler',
        'Execution Unit/Integer ALUs',
        'Execution Unit/Register Files/Floating Point RF',
        'Execution Unit/Register Files',
        'Execution Unit/Register Files/Integer RF',
        'Execution Unit/Results Broadcast Bus',
        'Instruction Fetch Unit/Branch Predictor/Chooser',
        'Instruction Fetch Unit/Branch Predictor/Global Predictor',
        'Instruction Fetch Unit/Branch Predictor',
        'Instruction Fetch Unit/Branch Predictor/L1_Local Predictor',
        'Instruction Fetch Unit/Branch Predictor/L2_Local Predictor',
        ...
        'Load Store Unit/LoadQ',
        'Load Store Unit',
        'Load Store Unit/StoreQ',
        'Memory Management Unit/Dtlb',
        'Memory Management Unit/Itlb',
        'Renaming Unit',
        'Renaming Unit/FP Front End RAT',
        'Renaming Unit/Free List',
        'Renaming Unit/Int Front End RAT'
    ]
    ...
    def get_per_core_total_power(mcpat_output_dict, unit_name, core_num):
        ...
        thermal_input_dict['Core' + str(core_num) + '/' + unit_name] = [
            total_dynamic_power,
            total_leakage_power
        ]
    ...
    def get_per_processor_total_power(mcpat_output_dict, unit_name):
        ...
        total_leakage_power = subthreshold_leakage + gate_leakage
        thermal_input_dict['Processor' + '/' + unit_name] = [
            runtime_dynamic,
            total_leakage_power
        ]
    ...
    for core_num in range(num_cores):
        ...
        total_leakage_power = subthreshold_leakage + gate_leakage
        thermal_input_dict['Core' + str(core_num)] = [
            runtime_dynamic,
            total_leakage_power
        ]
    ...
    inputfile_name = os.path.join(
        outputfile_dirname,
        "energystats-temp-" + timestep_ID + ".xml"
    )
    ...
    for core_num in range(num_cores):
        for parent_unit in all_per_core_parent_units:
            childrens_dynamic_power_vals = [
                power[0]
                for unit_name, power in thermal_input_dict.items()
                if ('Core' + str(core_num) + '/' + parent_unit + '/') in unit_name
            ]
            childrens_leakage_power_vals = [
                power[1]
                for unit_name, power in thermal_input_dict.items()
                if ('Core' + str(core_num) + '/' + parent_unit + '/') in unit_name
            ]
            ...
            thermal_input_dict['Core' + str(core_num) + '/' + parent_unit][0] = (
                thermal_input_dict['Core' + str(core_num) + '/' + parent_unit][0]
                - total_childrens_dynamic_power
            )

            # Update parent leakage power
            thermal_input_dict['Core' + str(core_num) + '/' + parent_unit][1] = (
                thermal_input_dict['Core' + str(core_num) + '/' + parent_unit][1]
                - total_childrens_leakage_power
            )
    ...
    for core_num in range(num_cores):
        ...
        # commented by Maz
        # thermal_input_dict['Core' + str(core_num) + '/Execution Unit/AVX512 Accelerator'] = [
        #     scaled_dynamic_power,
