...
doc = App.newDocument("Chip_core_sink_uniform_fin")
...
uniform_sink_width = chip_width
uniform_sink_height = 0.01
uniform_sink_depth = uniform_sink_width
uniform_sink_distance = 0.01

uniform_sink = Part.makeBox(uniform_sink_width, uniform_sink_height, uniform_sink_depth)
uniform_sink = uniform_sink.translate(App.Vector(0, uniform_sink_distance, 0))

chip = chip.cut(uniform_sink)
...
sink_width = hotspot_width
sink_height = uniform_sink_height
sink_depth = hotspot_width
sink_distance = uniform_sink_distance

hotspots = [0] * ncores_row * ncores_row
sinks = [0] * ncores_row * ncores_row
...
for i in range(ncores_row):
    for j in range(ncores_row):
        ...
        cores[i * ncores_row + j] = cores[i * ncores_row + j].cut(uniform_sink)
        sinks[i * ncores_row + j] = Part.makeBox(sink_width, sink_height, sink_depth)
        sinks[i * ncores_row + j].translate(App.Vector(translation * (i + 1) + i * core_width + translation2, sink_distance, translation * (j + 1) + j * core_width + translation2))
        hotspots[i * ncores_row + j] = hotspots[i * ncores_row + j].cut(sinks[i * ncores_row + j])
        uniform_sink = uniform_sink.cut(sinks[i * ncores_row + j])
        ...
        obj = doc.addObject("Part::Feature", "Sinks")
        obj.Shape = sinks[i * ncores_row + j]

obj = doc.addObject("Part::Feature", "Chip")
obj.Shape = chip
obj = doc.addObject("Part::Feature", "Uniform_sink")
obj.Shape = uniform_sink
