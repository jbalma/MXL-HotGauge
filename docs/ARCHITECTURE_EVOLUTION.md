...
| A design changed *because* of a measurement, then re-measured | **done 9 Sep (D1, §P0.22.3)** — the execution cluster made 2× / 4× denser *because* the rescue ladder said the array closes the concentration penalty, re-measured at matched watts: under the per-block planner shape (§P0.34) the laser holds the 2× cluster to the 2.40-equivalent rung and the 4× cluster to 2.00, 0 tiles capped, at 1.1–1.7× the reference's cooling die-wide (the seed shape's 202 / 162 W and 1.2–6.9× were the planner's); each loses the top rung. One die, one workload, one change: the loop has taken its first step, not run its course |
...
| A rung of the argued ladder falsified by its own measurement | **done 11 Sep (X4, §P0.27.4)** — the gen-3 two-die stack as argued (storage die on the sink side, one array) costs the whole die at every bond, exactly the monolithic result; the storage die must be off the compute die's heat path (ARGUED, needs a two-sink stack) |
