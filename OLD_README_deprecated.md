...
2) Set up 3d-ice (the thermal simulator)
    1) Download and patch 3d-ice (the thermal simulator) using `get_and_patch_3DICE.sh`
    2) If on a RHEL 6 machine also apply `HotGauge.3D-ICE.ThermalInit.RHEL_6.patch`
    3) Compile and test 3d-ice
        1) make sure compatible version of bison, flex, and superLU are installed. See compilation
        hints below for more details
        2) Compile the heatsink_plugin using `make` in *./3d-ice/heatsink_plugin/*
        3) Compile 3D-ICE executables using `make` in *./3d-ice/*
        4) Test 3D-ICE executables using `make test`
3) Try out the scripts in the *./examples/* directory. Running `python custom_simulation_with_warmup.py` in
...
encounter them. Some aspects of the patch may not fully apply. Some errors can be solved by manually applying failed patches. The details of the changes made by this patch file are given below.
